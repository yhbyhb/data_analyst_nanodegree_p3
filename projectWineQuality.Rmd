...
The sulphates has outliers above 1.5 g/dm^3 and has peak around 0.6. Ignored above 1.5 g/dm^3 as outliers
...
```{r echo=FALSE, mid_quality}
table(wineQuality$quality == 5 | wineQuality$quality == 6)
```

82.5% (1319 / 1599) are mid quality(5 and 6). 
...
Created "quality_class" factord variable for better visualization of quality histogram.
...
```{r echo=FALSE, alcohol_and_quality_class}
ggplot(aes(x = quality_class, y = alcohol), data = wineQuality) +
  geom_jitter(colour = 'black', fill = 'black',  alpha = 1/10, pch=21, cex=2)
```

Since the scatterplot is overplotted, I used jitter and alpha for a better visual.

```{r echo=FALSE, alchol_and_quality_class_boxplot}
qplot(x = quality_class, y = alcohol, data = wineQuality, geom="boxplot")
```

I used factored variable 'quality_class' for boxplot. The boxplot shows that medians alcohol value of each quality have positive slope.

```{r echo=FALSE, alchol_and_quality_density}
qplot(alcohol, data = wineQuality,
...
```{r echo=FALSE, alcohol_and_quality_linear_model}
...
```{r echo=FALSE, sulphates_and_quality}
ggplot(aes(x = quality_class, y = sulphates), data = wineQuality) +
...
```{r echo=FALSE, volatile.acidity_and_quality}
ggplot(aes(x = quality_class, y = volatile.acidity), data = wineQuality) +
  geom_jitter(colour = 'black', fill = 'black',  alpha = 1/5, pch=21, cex=2)
...
```{r echo=FALSE, residual.sugar_and_quality}
ggplot(aes(x = quality_class, y = residual.sugar), data = wineQuality) +
  geom_boxplot() +
...
```{r echo=FALSE, chlorides_and_quality}
ggplot(aes(x = quality_class, y = chlorides), data = wineQuality) +
  geom_boxplot() +
...
### pH and quality
```{r echo=FALSE, pH_and_quality}
ggplot(aes(x = quality_class, y = pH), data = wineQuality) +
  geom_boxplot()

# linear model. Remove outlier exceed 0.99
wine_pH_subset = subset(wineQuality,
                        pH <= quantile(wineQuality$pH, 0.99))
qplot(x = pH, y = quality, data = wine_pH_subset,
      color = I('black'), alpha = I(1/5)) +
  geom_smooth(method = 'lm', se = T, color = 'blue')

pHQualityLM <- lm(quality ~ pH, data = wine_pH_subset)
summary(pHQualityLM)
```

I thought pH is related with sour taste. linear model show very low R-squared value: 0.005245. However, I'll check another acidity variables. (volatile.acidity is already investigated above.)

### citric.acid and quality
```{r echo=FALSE, fixed.acidity_and_quality}
ggplot(aes(x = quality_class, y = citric.acid), data = wineQuality) +
  geom_boxplot()
...
```{r echo=FALSE, fixed.acidity_and_quality}
ggplot(aes(x = quality_class, y = fixed.acidity), data = wineQuality) +
  geom_boxplot()
...
qplot(x = fixed.acidity, y = quality, data = wine_f.acid_subset,
      color = I('black'), alpha = I(1/5)) +
  geom_smooth(method = 'lm', se = T, color = 'blue')

fixed.acidityQualityLM <- lm(quality ~ fixed.acidity, data = wine_f.acid_subset)
summary(fixed.acidityQualityLM)
```

R-squred value of fixed.acidity and quality is slighty higher than pH and quality. It is not highly related with quality.
...
```{r echo=FALSE, free.sulfur.dioxide_and_total.sulfur.dioxide}
...
```{r echo=FALSE, volatile.acidity_and_pH}
...
```{r echo=FALSE, fixed.acidity_and_pH}
...
### citric.acid and pH
```{r echo=FALSE, citric.acid_and_pH}
...
```{r echo=FALSE, alcohol_volatile.acidity_and_quality}
...
```{r echo=FALSE, alcohol_volatile.acidity_and_quality_facet_warp}
ggplot(aes(x = alcohol, y = volatile.acidity), 
       data = wineQuality) +
...
```{r echo=FALSE, alcohol_volatile.acidity_and_quality_LM}
...
```{r echo=FALSE, alcohol_sulphates_and_quality}
...
```{r echo=FALSE, alcohol_sulphates_and_quality_LM}
...
```{r echo=FALSE, volatile.acidity_sulphates_and_quality}
...
```{r echo=FALSE, volatile.acidity_sulphates_and_quality_LM}
...
## similar variables investigation
I'd like to continue from bivariate plots by adding quality_class variable. 
First, sulfur dioxide famliy (free.sulfur.dioxide and total.sulur.dioxide)

### sulfur dioxide famliy
```{r echo=FALSE, sulfur_dioxide_famliy}
...
#   ggtitle('Relationship between alcohol and sulphates by quality') +
#   xlab('alcohol (%)') +
#   ylab('sulphates (g / cm^3)')
